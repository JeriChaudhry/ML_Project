...
def plot_transformed_images(image_paths, transform, n=3, seed=42):
    ...
    Will open in image paths from image_paths, transform them
...
def Plot_imgs(image_path):
    ...
    # Set seed
    random.seed(93) # <- try changing this and see what happens
...
def display_random_images(dataset: torch.utils.data.dataset.Dataset,
    ...
    if n > 8:
        n = 8
        display_shape = False
        print(f"For display purposes, number of sample images shouldn't be larger than 8, setting to 8 and removing shape display.")
